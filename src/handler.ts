...
module.exports.retrievePollingPlace = async (event: LexEvent): Promise<LexResponse> => {
  const { StreetAddress, ZipCode, County } = event.currentIntent.slots;
  ...
  return buildSuccessfulResponse(
    `Your polling place is ${pollingLocation.name}, which is located at: ${pollingLocation.address}`
  );
};
